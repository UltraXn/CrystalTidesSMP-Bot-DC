...
async function handleLinkMinecraft(interaction: ChatInputCommandInteraction, sourceId: string, playerName: string, codeInput: string) {
    const discordId = interaction.user.id;
    const discordTag = interaction.user.tag;

    await supabase.from('profiles').update({ social_discord: null, discord_tag: null }).eq('social_discord', discordId);
    await supabase.from('profiles').update({ social_discord: null, discord_tag: null }).eq('minecraft_uuid', sourceId);
...
async function handleLinkWeb(interaction: ChatInputCommandInteraction, sourceId: string, codeInput: string) {
    const discordId = interaction.user.id;
    const discordTag = interaction.user.tag;
